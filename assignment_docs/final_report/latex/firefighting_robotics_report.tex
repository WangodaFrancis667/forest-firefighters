...
This project implements an autonomous forest firefighting mission in the Webots Forest Firefighters simulation environment. The mission objective is to use aerial robots to patrol a forest, detect fire or smoke using onboard cameras, navigate toward the detected fire, and extinguish it by dropping water. The final solution uses eight Mavic drone controllers deployed at four forest corners (two drones per corner), providing maximum forest coverage, faster response to multiple fires, and system redundancy.

The implementation was executed in a stable Webots R2021b Docker environment with NVIDIA GPU OpenGL rendering. Before each mission test, a preflight checker verified that all eight drones started from the correct base positions at their assigned corners:
\begin{itemize}
    \item \textbf{SW Corner}: Drones 1 \& 2 at $(4.0, 4.0, 17.4)$ and $(4.0, 3.0, 17.43)$
    \item \textbf{NW Corner}: Drones 3 \& 4 at $(2.0, 24.0, 17.92)$ and $(2.0, 26.0, 18.82)$
    \item \textbf{SE Corner}: Drones 5 \& 6 at $(26.0, 3.0, 15.25)$ and $(28.0, 3.0, 16.33)$
    \item \textbf{NE Corner}: Drones 7 \& 8 at $(28.0, 26.0, 19.34)$ and $(28.0, 28.0, 17.98)$
\end{itemize}
...
\node[block, below right=of world] (drones) {Eight Mavic\\Drones\\(4 Corners)};
...
\section{Conclusion}
The final system demonstrates an autonomous aerial firefighting mission using eight Mavic drones deployed at four forest corners in Webots. The drones patrol their assigned forest quadrants, detect smoke/fire with camera-based OpenCV processing, navigate toward detected fire regions, drop water, and continue patrolling for additional propagated fires. The clean primary runs confirm stable operation with multiple detections and water drops across all drones, satisfying the mission requirements for locomotion, perception, navigation, integration, and documentation.
