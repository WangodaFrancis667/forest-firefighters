...
\node[process] (gait) at (2.45,-4.75) {Spot Gait Loop\\continuous motion};
...
The Spot-like robot is retained from the original project baseline as a ground support robot. Its controller keeps it alive for the full mission, cycles a simple gait, and accepts a keyboard-triggered water burst. In the final architecture, Spot is not the main search platform; it provides visible legged-robot integration and a secondary water-delivery path while the Mavic drones perform wide-area patrol and visual detection.
